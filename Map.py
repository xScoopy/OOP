class Map:
    ...
    max players is a private attribute to keep the game integrity clean. That's one we really don't want altered.
    the safezone, max traitors/survivors are protected since we want those to stay constant, but they are available for alterations
    with possible different game modes in the future."""
    _safezone = "cabin"
    __max_players = 8
    _max_traitors = 3
    _max_survivors = 6
    ...
    def add_player(self, player):
        ...
        if len(self.players) <= Map.__max_players:
